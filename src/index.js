...
for (let key in IE8InitMapping) {
	IE8[key] = function (typeArg, eventInit) {
		...
		for (let optKey in eventInit) {
			...
			if (isExist) {
				if (optKey === 'bubbles') {
					event.cancelBubble = !eventInit.bubbles;
				} else if (optKey === 'cancelable') {
					event.returnValue = !eventInit.cancelable;
				...
				} else if (optKey === 'target') {
					event.srcElement = eventInit.target;
				} else {
...
let isECS = true,
	canUseKeyboardEvent = true,
	canUseInputEvent = true;
...
try {
	...
	canUseKeyboardEvent = false;
}
try {
	new InputEvent('input', {});
} catch (_) {
	canUseInputEvent = false;
}
